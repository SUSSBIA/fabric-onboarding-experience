...
Microsoft Fabric is a shared analytics platform. Users may interact with reports, semantic models, Lakehouses, pipelines, notebooks, dataflows, warehouses, connections, refresh schedules, and institutional data.

This means users should understand their responsibilities before they start creating, editing, sharing, exporting, or publishing Fabric assets.
...
* Whether the data or asset is labelled `Confidential - SUSS`, `Restricted - SUSS`, `Unrestricted - SUSS`, or `None`
* What they are allowed to share
* Whether they are working in a personal, sandbox, department, or BIA production workspace
* Who owns the workspace or asset
* Whether a department workspace has both an owner and deputy owner
...
| Workspace accountability    | Each non-personal workspace should have a clear owner or responsible group                                                   |
| Ownership continuity        | Department workspaces should have both a workspace owner and deputy workspace owner                                          |
...
Fabric workspaces use role-based access.

The exact permissions may depend on the item and workspace configuration, but users will generally encounter the following roles.
...
Having a higher workspace role does not mean a user should freely access, export, or share all data.

Users must still follow sensitivity label, sharing, connection, refresh, workspace boundary, and production expectations.
...
In the current operating model, BIA controls the relevant Fabric tenant-level settings for the University’s analytics environment. This means some capabilities may not be available to users even if they have workspace access.
...
In the current environment, users may see sensitivity labels such as:
...
Sharing should follow the principle of least privilege.

Users should share only what is needed, with only the people who need it.

## BIA production output sharing
...
This means non-BIA users may be able to consume approved production outputs, but they should not be added directly to the BIA Production Workspace.
...
RLS should be designed, tested, and validated carefully before reports are shared more widely.

Users should not assume that workspace access alone is sufficient to control what data each user can see inside a report.
...
## Workspace boundaries and security expectations

Different workspace types carry different expectations.

| Workspace Type           | Security Expectation                                                                                                                                                                                                   |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Personal Workspace       | Used for individual exploration and private drafts. It should not be used for shared department assets or production work.                                                                                             |
| Sandbox Workspace        | Used for safe learning and experimentation with public, mocked, synthetic, or approved non-sensitive data. Outputs are not official.                                                                                   |
| Department Workspace     | Used for approved department-level exploration, prototyping, and working assets. Department workspaces should have both a workspace owner and deputy workspace owner. Outputs are not automatically production assets. |
| BIA Production Workspace | Used for BIA-managed production analytics assets. Direct workspace access is restricted to BIA users. Non-BIA users should consume approved outputs through approved report or app sharing channels.                   |

Users should not treat personal, sandbox, or department workspace outputs as official production assets unless they have gone through appropriate review.
...
* [ ] I know whether I am in a personal, sandbox, department, or BIA production workspace
...
* [ ] I know whether a department workspace has both a workspace owner and deputy workspace owner
* [ ] I understand whether my output is experimental, department-facing, or production-facing
* [ ] I know that direct BIA Production Workspace access is restricted to BIA users
* [ ] I know that non-BIA users should consume approved production outputs through approved report or app sharing channels
